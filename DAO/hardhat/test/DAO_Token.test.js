...
describe('DAOToken Test', function () {
    ...
    describe("Deployment", function () {
        it("Should set the right owner", async function () {
            const { govToken, owner } = await loadFixture(deployAwardFixture)
            expect(await govToken.owner()).to.equal(owner.address);             //.owner() in Ownable
        });
    ...
    it("Should transfer tokens between accounts", async function () {
        ...
        const { govToken, owner, addr1, addr2 } = await loadFixture(deployAwardFixture)
    ...
    describe("Voting", function () {
        it("Should track votes correctly", async function () {
            ...
            const { govToken, owner, addr1 } = await loadFixture(deployAwardFixture)

            await govToken.delegate(owner.address);      // Delegate voting rights to self (can delegate to others as well)
